...
(function (root, factory) {
  ...
  const KEYWORDS = {
    cpp: [
      ...
      'inline', 'namespace', 'new', 'operator', 'private', 'protected', 'public', 'return',
      'sizeof', 'static', 'struct', 'switch', 'template', 'this', 'throw', 'try', 'typedef',
      'typename', 'union', 'using', 'virtual', 'volatile', 'while', '#include', '#define',
      '#ifdef', '#ifndef', '#endif', 'constexpr', 'nullptr', 'override', 'final'
    ],
    python: [
    ...
    typescript: [
      ...
      'if', 'import', 'in', 'instanceof', 'let', 'new', 'return', 'super', 'switch', 'this',
      'throw', 'try', 'typeof', 'var', 'void', 'while', 'with', 'yield', 'true', 'false',
      'null', 'undefined', 'type', 'interface', 'namespace', 'readonly', 'implements',
      'enum', 'as', 'any', 'string', 'number', 'boolean', 'unknown', 'never'
    ],
    sql: [
      'select', 'from', 'where', 'insert', 'into', 'update', 'delete', 'create', 'table',
      'drop', 'alter', 'join', 'inner', 'left', 'right', 'outer', 'on', 'group', 'by',
      'order', 'asc', 'desc', 'having', 'limit', 'as', 'distinct', 'count', 'sum', 'avg',
      'max', 'min', 'like', 'in', 'and', 'or', 'not', 'is', 'null', 'primary', 'key'
    ],
    bash: [
      'if', 'then', 'else', 'elif', 'fi', 'for', 'while', 'do', 'done', 'in', 'case',
      'esac', 'function', 'return', 'exit', 'export', 'source', 'alias', 'cd', 'ls', 'echo',
      'mkdir', 'rm', 'npm', 'node', 'git', 'curl', 'sudo', 'chmod', 'chown', 'cat', 'grep'
    ]
  };

  const TYPES = {
    cpp: ['int', 'float', 'double', 'char', 'void', 'bool', 'long', 'short', 'unsigned', 'signed', 'size_t', 'string', 'vector', 'map', 'set', 'pair', 'unordered_map', 'unordered_set', 'cout', 'cin', 'endl', 'std'],
    python: ['int', 'float', 'str', 'bool', 'list', 'dict', 'tuple', 'set', 'print', 'len', 'range', 'enumerate', 'zip', 'map', 'filter', 'open'],
    javascript: ['Promise', 'Array', 'Object', 'String', 'Number', 'Boolean', 'Map', 'Set', 'console', 'JSON', 'Math', 'Date', 'RegExp', 'Error', 'window', 'document', 'process'],
    typescript: ['Promise', 'Array', 'Object', 'String', 'Number', 'Boolean', 'Map', 'Set', 'console', 'JSON', 'Record', 'Partial', 'Pick', 'Omit'],
  };
  ...
  function highlight(code, lang) {
    ...
    const highlightedLines = lines.map((line) => {
      ...
      if (nLang === 'python' || nLang === 'bash') {
        ...
        if (hashIdx !== -1) {
          commentPrefix = line.slice(0, hashIdx);
        ...
        if (slashIdx !== -1) {
          commentPrefix = line.slice(0, slashIdx);
